...
export class WebServer extends Service {
  static Config: z<Config> = z.object({
    // A required non-empty string: any address the OS can bind, so a deployment
    // can serve one interface. A value that is not a valid listen address fails
    // at listen time with the OS's own error rather than as a config error here.
