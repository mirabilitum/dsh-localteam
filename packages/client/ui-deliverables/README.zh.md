...
部署在共享团队上时，已定稿的助手消息操作行在"复制"旁边还会多出两个控件：**打包下载本会话产出** 与 **移交控制权**。前者先向主机询问这个会话的项目产出了什么，再把 `work\` 打成一个 ZIP——包内结构与服务器一致，不含 `input\`、`build\`、`logs\`，也不含会话自己的 `temp\`。后者列出部署中的其他成员，并提供"释放控制权让任何人接手"；主机对两者的准入都只认**当前驱动该会话的成员**。两者都是对主机团队路由的**同源普通 HTTP 请求**，因此这里不需要 Remote，也不需要任何服务：没有这些路由的部署会回答 404，插件据此判定为单机实例，**一个控件都不渲染**。

保存属于"读"。任何已登录成员都能保存自己本来就能在预览里看到的文件，且这个动作不会改变控制权。
